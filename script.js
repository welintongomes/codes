...
<body>
  <div class="container">
    ...
    <div id="menu">
      ...
      <select id="tabuada">
        ...
        ${[...Array(10).keys()].map(i => `<option value="${i + 1}">Tabuada do ${i + 1}</option>`).join('')}
      </select>
      <br>
    ...
    <div id="game">
      <h2>Questão <span id="questaoNumero">1</span> de 10</h2>
      <div id="pergunta" style="font-size: 22px; margin-bottom: 15px;"></div>
    ...
    <div id="result">
      <h2>Fim do Jogo!</h2>
      <p>Sua pontuação final: <strong id="pontuacaoFinal"></strong> de 10</p>
  ...
  <script>
    ...
    function iniciarJogo() {
      ...
      numeroTabuada = parseInt(tabuadaSelect.value);

      if (isNaN(numeroTabuada)) {
        alert("Por favor, selecione uma tabuada.");
        return;
      }

      multiplicadores = [...Array(10).keys()].map(n => n + 1).sort(() => Math.random() - 0.5);
      questaoAtual = 1;
      pontuacao = 0;
    ...
    function proximaPergunta() {
      const multiplicadorAtual = multiplicadores[questaoAtual - 1];
      respostaCorreta = numeroTabuada * multiplicadorAtual;

      document.getElementById('questaoNumero').textContent = questaoAtual;
      document.getElementById('pergunta').textContent = \`Quanto é \${numeroTabuada} × \${multiplicadorAtual}?\`;

      gerarOpcoesEnganosas(numeroTabuada, multiplicadorAtual, respostaCorreta);
      document.getElementById('feedback').textContent = '';
    }

    function gerarOpcoesEnganosas(a, b, correta) {
      ...
      while (opcoes.size < 4) {
        let fake;
        const tipoErro = Math.floor(Math.random() * 4);
        switch (tipoErro) {
          case 0: fake = (a + 1) * b; break;
          case 1: fake = a * (b + 1); break;
          case 2: fake = a + b; break;
          case 3: fake = (a - 1 > 0 ? a - 1 : a + 2) * b; break;
        }
        if (fake !== correta && fake > 0) opcoes.add(fake);
      }

      const embaralhadas = Array.from(opcoes).sort(() => Math.random() - 0.5);
    ...
    // Método para alternar modo de tela cheia
    toggleFullscreen() {
        ...
        if (mainContent.classList.contains('fullscreen-mode')) {
            ...
            // Expande o iframe para ocupar toda a janela
            ...
            // Adiciona botão flutuante para sair da tela cheia
            ...
            // Restaura o conteúdo da ferramenta
            const toolContent = document.getElementById('tool-content');
            if (toolContent) {
    ...
    renderToolsList(toolsToRender = this.tools) {
        ...
        if (toolsToRender.length === 0) {
            // Verifica se emptyState existe antes de tentar adicioná-lo
            if (emptyState) {
                // Clone o emptyState para evitar problemas de referência
                const emptyStateClone = emptyState.cloneNode(true);
                toolsContainer.appendChild(emptyStateClone);
            } else {
                // Cria um elemento de estado vazio se não existir
